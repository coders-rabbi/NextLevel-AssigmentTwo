...
const getASpecificOrdersController = async (req: Request, res: Response) => {
  try {
    const { userId } = req.params;
    const result = await UserService.getAspecificOrderIntoDB(userId as string);
    res.status(200).json({
      success: true,
